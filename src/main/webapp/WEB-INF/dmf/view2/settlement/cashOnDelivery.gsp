...
<g:if test="${isOK}">
	<g:form name="updateForm" action="bankupdate" onsubmit="return false;">
		<input type="hidden" id="orderNo" name="orderNo" value="${orderNo?.encodeAsHTML()}" />	
			...
			<!-- <iframe src="${g.resource(dir:'static/dmol/users', file:'cashOnDeliveryHelp.html')}" frameborder="0" scrolling="no" width="100%" height="100%" marginheight="0"></iframe> -->
			
			<g:render template="/common/fragment/cashOnDeliveryHelp" />
	 		<h2>代金引換決済の内容確認</h2>
			...
			<table cellpadding="0" cellspacing="0" class="commonTable1 mB5 w664">
				<tr>
					<th width="180">代引手数料(税込)</th>
					<td class="pL10">360円</td>
				</tr>
			</table>		
	 		</g:else>
			<table cellpadding="0" cellspacing="0" class="commonTable1 mB20 w664">
				<tr>
					<th width="180">お支払い金額合計（税込）</th>
					<td class="pL10"><g:formatNumber number="${totalAmountFee?.encodeAsHTML()}" format="#,###" />円</td>
				</tr>
			</table>		
...
<g:set var="errormessage" value="${flash.message ?: '注文データを取得できませんでした。'}" />
			...
			<g:if test="${errormessage?.encodeAsHTML()=='不正の画面遷移：代引決済を選択できません。'}">
			<br/>
...
$("#nextBtn").click(function() {
	...
	//第三期 2012/03/19 by A&I End
	param.orderNo = document.getElementById('orderNo').value;
	var url = '${createLink(controller:'settlement', action:'cashOnDeliveryUpdate')}';
