<html>
	<head>
		...
		<script type="text/javascript" src="${resource(dir:'js/jquery', file:'jquery.balloon.js')}"></script>
		<script>
		...
		<script>
			...
			function getOrderInfoList() {
				...
				param.corporateName = $('#corporateName').val();
				...
				param.settlementStatusPoi = $('#settlementStatusPoi').val();
				param.examinationStatus = $('#examinationStatus').val();
				param.printStatus = $('#printStatus').val();
				$.post("search", param, function(data) {
					...
					var infoCNT = 0;
					var hasSErr = false;
					$.each(data.list, function(infoCNT, OrderInfo) {
						infoCNT = infoCNT + 1;
    					var htmlDetailNo = "";
						...
						$.each(OrderInfo.details, function(detailCNT, OrderDetail) {
							h = h + 1;
							var isLastDetail = (detailCNT == (OrderInfo.details.length - 1));
							var lastDetailClass = "tbpadding";
<%--						/* 伝票NO, 決済エラーの配列化 & 最大行数算出 */--%>
							var detailRowCount = 1;
							var sErrNum, trackNum;
							var sErrCount;
							if($.isEmptyObject(OrderDetail.settlementError)) {
								sErrNum = []; sErrCount = 0;
							} else {
								sErrNum = OrderDetail.settlementError.split("|");
								sErrCount = 1;
								hasSErr = true;
							}
							if($.isEmptyObject(OrderDetail.trackingNumber))
								trackNum = [];
							else
								trackNum = OrderDetail.trackingNumber.split("/");
							detailRowCount = Math.max(sErrCount, trackNum.length);/*<%--最大値を明細内部行数とする--%>*/
							...
							htmlDetailNo += '<div class="tbpadding tbunderlineorder"  id="Y'+infoCNT+'D'+h+'"><a onclick="return setSession(this.href)" class="order" " href="<g:createLink controller="orderDetail" action="show"></g:createLink>/' + OrderDetail.id + '">' + turnNull(OrderDetail.detailNo) + '</a></div>';
							...
							/* create detail inner div */
							for(var i=0; i<detailRowCount; i++) {
								...
								htmlPrintStatus += '<div class="' + lastDetailClass + '">' + turnNull(trackNum.length>i? trackNum[i]: null) + '</div>';

								if (sErrCount && i==0) {
									var sErrTitle = "";
									$.each(sErrNum, function(i, v) {
										sErrTitle += v.replace("/", "：") + "<br>";
									});
									htmlSettlementStatus += '<div class=' + lastDetailClass + '"><a href="javascript:void(0)" title="' + sErrTitle + '" class="balloon">エラー内容</a></div>';
								} else {
						...
						html += '<td id="Y'+infoCNT+'Order" style="word-wrap:break-word;word-break:break-all; ">' + turnNull(OrderInfo.orderNo) + '</td>';
						...
						html += '<td style="word-wrap:break-word;word-break:break-all; ">' + HtmlEncode(turnNull(OrderInfo.corporateName)) + '</td>';
			...
			function turnNull(cluValue) { return $.isEmptyObject(cluValue)? "&nbsp;": cluValue;}
			//初期化
			$(document).ready( function() {
				//ソートボタンとページングの初期化
	...
	<body>
	<div class="mainBlock">
			<h1 align="left">請求書支払い管理</h1>
			
<g:render template="/common/fragment/flashmessages" />
			<div class="searchConditiontoggler"><a id="searchConditiontoggler" class="button" href="javascript:void(0)"><span class="icon-conditiontoggler">検索条件</span></a>
			</div>
			<div class="dialog searchConditionFormArea" id="searchConditionFormArea">
				...
				<table>
					<tbody>
						<tr class="prop">
							<td class="name" title="半角数字で指定！">注文No</td>
							<td class="value">
								<input type="text" id="orderNo" name="orderNo" size="50" maxlength="15" title="hogehoge" value="${params?.orderNo?.encodeAsHTML()}" /><div style="padding-top:1px;">&nbsp;（例）12345678901234</div>
							</td>
						</tr>
						<tr class="prop">
						...
						<tr class="prop">
							...
							<td class="value">
								<span id="corporateNameCheck">
								<input type="text" id="corporateName" name="corporateName" size="100" maxlength="256" value="${params?.corporateName?.encodeAsHTML()}" /><br>&nbsp;（例）エー･アンド・アイ システム
								</span>
							</td>
						</tr>
						<tr class="prop">
						...
						<tr class="prop">
							...
							<td class="value">
								<g:select id="settlementStatusPoi" name="settlementStatusPoi" value="${params?.settlementStatusPoi}"
									from="${settlementStatusPoiList}"
									noSelection="${['null':'すべて']}"
									optionKey="id" optionValue="name" >
								</g:select>
							</td>
						...
						<tr class="prop">
							...
							<td class="value">
								<g:select id="printStatus" name="printStatus" value="${params?.printStatus}"
									from="${printStatusList}"
									noSelection="${['null':'すべて']}"
									optionKey="id" optionValue="name" >
								</g:select>
							</td>
					...
					<table style="table-layout:fixed;">
						...
						<thead>
							...
							<g:else>
								...
								<th rowspan="2" class="vhcenter">会社名</th>
