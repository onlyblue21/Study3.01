...
<!-- breadcrumb -->
	<div class="breadcrumb bgGrad">
		<ul>
			...
			<li>アイテム（テンプレート）の編集</li>
		</ul>
	</div>
<!-- /breadcrumb END -->

	<div class="mainBlock">
		<h1>アイテム（テンプレート）の編集</h1>
...
<g:render template="/common/fragment/validatormessages" />
		...
		<g:form method="post" action="update" name="updateForm" onsubmit="return false;">
			<input type="hidden" name="id" value="${itemInfo?.id?.encodeAsHTML()}" />
			<input type="hidden" name="version" value="${itemInfo?.version?.encodeAsHTML()}" />
			<div class="dialog">
				<table>
					<tbody>
						...
						<tr class="prop">
							...
							<td valign="top" class="value ${hasErrors(bean:itemInfo,field:'itemGenre.id','errors')}">
								<g:select id="genreId" name="itemGenre.id" from="${itemGenreList}" optionKey="id" optionValue="genreName" value="${itemInfo?.itemGenre?.id}" />
							</td>
						</tr> 
...
<g:each in="${seasonCategoryList}" status="i" var="seasonCategory">
						...
						<tr class="prop">
							<td valign="top" class="name">${fieldNames?.ItemInfo?.dateCreated?.encodeAsHTML()}:</td>
							<td valign="top" class="value"><g:formatDate format="${timestampFormat}" date="${itemInfo?.dateCreated}" /></td>
						</tr>

						<tr class="prop">
							<td valign="top" class="name">${fieldNames?.ItemInfo?.lastUpdated?.encodeAsHTML()}:</td>
							<td valign="top" class="value"><g:formatDate format="${timestampFormat}" date="${itemInfo?.lastUpdated}" /></td>
						</tr>
