...
<g:set var="omoteTitleAlt" value="あて名面の" />
<g:if test="${itemGenre.genreId == 'dmol_enclosed_leaf'}">
<g:set var="omoteTitleAlt" value="" />
</g:if>
			<h2>${omoteTitleAlt}デザインを選ぶ<span>過去に作成したデザインからご希望のものをお選びください。</span></h2>
			<p>画像をクリックすると、デザイン確認画面を表示します。</p>
...
<g:form action="selectMyLibrary" name="confirmSelectedMyLibraryForm">
<input type="hidden" name="myLibraryPageInfoId" value="" />
...
var confirmSelectedMyLibrary = function(ioffset, myLibraryPageInfo) {
	...
	document.confirmSelectedMyLibraryForm.myLibraryPageInfoId.value = myLibraryPageInfo.id;
...
var updateMyLibraryPageInfoList = function(offset) {
	...
	var sparams = {};
	sparams.genreId = "${genreId?.encodeAsHTML()}";
