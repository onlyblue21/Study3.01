...
<!-- /breadcrumb END -->
	...
	<div class="mainBlock">
		...
		<div class="buttons">
			<span class="button"><a href="javascript:void(0)" id="createBtn" class="button"><span class="icon-create">ユーザの追加</span></a></span>
		</div>
...
<sec:ifAllGranted roles="AUTHORITY_JPMD_MANAGE_USER">
		...
		<div class="list">
			<table>
				<thead>
					<tr>
						<g:sortableColumn property="userId" title="${fieldNames?.User?.email?.encodeAsHTML()}" params="${params}" />
						<g:sortableColumn property="role" title="${clazzNames?.Role?.encodeAsHTML()}" params="${params}" />
...
<g:if test="${!userList || (userList.size() <= 0)}">
					...
					<tr>
						...
						<td>
							<div class="tbpadding tbunderline">${user.role?.roleName.encodeAsHTML()}</div>
