...
<g:form method="get" action="confirm" name="prevForm" />
...
$("#nextBtn").click(function() {
	pleaseWait();
	$.ajax({
		url: "${createMyLibraryPath}",
		success: function(data, textStatus, jqXHR) {
			if (data.result == "success") {
				return isEditing(data.id);
			} else {
				$().toastmessage('showErrorToast', data.message ? data.message : "デザインを作成することが出来ませんでした。");
				closeWait();
			}
			return false;
		}
