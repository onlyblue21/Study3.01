...
<g:render template="/common/fragment/header"/>	
	...
	<!-- commonContents -->
<g:if test="${isOK}">
	<g:form name="updateForm" action="creditcardupdate" onsubmit="return false;">
		<input type="hidden" id="cardNo" name="cardNo" value="${requestRetMap?.cardNo?.encodeAsHTML()}" />
		<input type="hidden" id="cardExp" name="cardExp" value="${requestRetMap?.cardExp?.encodeAsHTML()}" />
		<input type="hidden" id="secCd" name="secCd" value="${requestRetMap?.secCd?.encodeAsHTML()}" />
		<input type="hidden" id="birthDay" name="birthDay" value="${requestRetMap?.birthDay?.encodeAsHTML()}" />
		<input type="hidden" name="cardNoM" value="${requestRetMap?.cardNoM?.encodeAsHTML()}" />
			<div class="commonContents w664">
			<img src="${resource(dir:'/static/dmol/users/images',file:'layout')}/c_chart_4.gif" class="mB20" />
	 		<h2>クレジットカード決済の内容確認</h2>
			...
			<table cellpadding="0" cellspacing="0" class="commonTable1 mB20">
				<tr>
					<th width="180">カード番号</th>
					<td>${requestRetMap?.cardNoM?.encodeAsHTML()}&nbsp;
					</td>
				</tr>
			</table>
...
$("#nextBtn").click(function() {
	...
	param.cardExp = document.getElementById('cardExp').value;
	param.secCd = document.getElementById('secCd').value;
	param.birthDay = document.getElementById('birthDay').value;
	// 第三期 2012/03/19 by A&I Start
	...
	// 第三期 2012/03/19 by A&I End
	var url = '${createLink(controller:'settlement', action:'creditcardupdate')}';
