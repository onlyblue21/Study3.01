	...
	<head>
		...
		<script>
			...
			function reviewConfirm(){
				...
				for(var i=0;i < collnum;i++){
					...
					if(colls[i].type.toLowerCase()== "checkbox" && colls[i].checked == true){
						splitCharInx = colls[i].name.indexOf("C");
						orderno = document.getElementById(colls[i].name.substring(0, (splitCharInx +1)).replace("C","Order")).innerText;
						if (colls[i].name.length - splitCharInx == 2) {
							detailno = "0" + colls[i].name.substring(splitCharInx+1, colls[i].name.length);
						} else {
							detailno = colls[i].name.substring(splitCharInx+1, colls[i].name.length);
						}
						arrOrderNo = arrOrderNo + "," + orderno + "D" + detailno;
					}
				}
				if(arrOrderNo!=""){
			...
			function getOrderInfoList() {
				...
				param.settlementStatusCod = $('#settlementStatusCod').val();
				...
				$.post("search", param, function(data) {
					...
					$.each(data.list, function(infoCNT, OrderInfo) {
						...
						$.each(OrderInfo.details, function(detailCNT, OrderDetail) {
							...
							row = row + 1;
							if (OrderDetail != null) {
								htmlShowCheckBox += divClass;
								if(OrderDetail.showCheckBox == "show"){
									htmlShowCheckBox += '<div class="tbpadding tbunderlineorder" align="center"><input name="Y' + infoCNT + 'C' + h + '" type="checkbox"/></div>';
								}else{
									htmlShowCheckBox += '<div class="tbpadding tbunderlineorder" align="center">&nbsp;</div>';
								}
								...
								if(OrderDetail.trackingNumber==null||OrderDetail.trackingNumber==""){
									...
									var trackCNT;
									for(trackCNT = 0 ; trackCNT<trackNum.length; trackCNT++){
										htmlPrintStatus += '<div class="' + lastDetailClass + '">' + turnNull(trackNum[trackCNT]) + '</div>';
										htmlShowCheckBox += '<div class="' + lastDetailClass + '">&nbsp;</div>';
				...
				<table>
					<tbody>
						...
						<tr class="prop">
							...
							<td class="value">
								<g:select id="settlementStatusCod" name="settlementStatusCod" value="${params?.settlementStatusCod}"
									from="${settlementStatusCodList}"
									noSelection="${['null':'すべて']}"
									optionKey="id" optionValue="name" >
								</g:select>
							</td>
