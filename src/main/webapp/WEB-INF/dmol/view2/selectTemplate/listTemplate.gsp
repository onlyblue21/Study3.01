...
<!-- commonContents -->
		<div class="commonContents flow">
			...
			<p>テンプレート画像をクリックすると、<g:if test="${showSearchCondition}">詳しい説明をご覧いただけます。</g:if><g:else>デザイン面のテンプレート選択に進みます。</g:else></p>

			<div class="temBox">

<g:if test="${showSearchCondition}">
				<div class="sideLeft">
...
<g:each in="${seasonCategoryList}" status="i" var="seasonCategory">
				...
				<div class="${showSearchCondition ? 'sideRight' : 'w716center'}">
...
<g:form action="selectTemplate" name="confirmSelectedTemplateForm">
<input type="hidden" name="itemInfoId" value="" />
...
var confirmSelectedTemplate = function(ioffset, itemInfo) {
	...
	document.confirmSelectedTemplateForm.itemInfoId.value = itemInfo.id;
...
var updateItemInfoList = function(offset) {
	...
	sparams.templateType = "${templateType?.encodeAsHTML()}";
